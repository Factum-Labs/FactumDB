...
import { A, AMB, GRN, GRY, ORG, RED } from '../lib/tokens'
...
export const cases: {
  ...
  { id: 'FDB-2026-014', name: 'Finance Balance Investigation', examiner: 'Analyst 01', files: 6, status: 'Analysed', tone: 'ok', opened: '2026-03-14 09:20' },
...
export const evidence = [
  ...
  { name: 'mysql-bin.index', type: 'Binary log index', size: '1 KB', hash: 'c40b8e77af31…6f5b', state: 'Verified' },
  { name: 'schema_notes.txt', type: 'Examiner note', size: '2 KB', hash: 'a71c04ffe8b2…19c6', state: 'Verified' },
]

/**
 * Log files named by the registered `mysql-bin.index`, as read at normalization.
 *
 * A log can only be *named* missing when the index lists it (R-COV-002). Without
 * an index nothing can be shown to be absent at all — coverage is merely unknown
 * (R-COV-001) — so it is this list, not the arithmetic gap between the filenames
 * we happen to hold, that licenses the finding. Empty until the index is parsed.
 */
export const binlogIndex = ['binlog.000018', 'binlog.000019', 'binlog.000020']

/** Index entries with no corresponding registered evidence file. */
export const missingLogs = binlogIndex.filter((n) => !evidence.some((e) => e.name === n))

// ── Pipeline ─────────────────────────────────────────────────────────────────

export const stages: { name: string; detail: string; time: string; kind: 'ok' | 'warn' }[] = [
  { name: 'Evidence registration', detail: '6 files', time: '0.4 s', kind: 'ok' },
  { name: 'SHA-256 hashing', detail: '6 hashes', time: '2.1 s', kind: 'ok' },
  { name: 'Verified working copies', detail: '6 / 6 match', time: '3.8 s', kind: 'ok' },
...
/**
 * 1-based stage that turns tool output into canonical objects. Coverage findings
 * are stated against those objects, so nothing that depends on the log index —
 * the missing-file banner, the coverage-gap tile — may be shown before it runs.
 */
export const NORMALIZATION_STAGE = stages.findIndex((s) => s.name === 'Normalization') + 1

// Counts only — the stat tiles render in uniform ink; a per-stat colour here
// reads as a status the number does not actually carry.
//
// `stage` is the 1-based pipeline stage that produces the figure; the tile reads
// "—" until that stage has run, so a partial run never shows a final count.
export const stats: { label: string; value: string; stage: number }[] = [
  { label: 'Evidence files', value: '6', stage: 1 },
  { label: 'Verified', value: '6', stage: 3 },
  { label: 'Tables parsed', value: '2', stage: 5 },
  { label: 'Coverage gaps', value: '1', stage: NORMALIZATION_STAGE },
  { label: 'Transactions', value: '4', stage: 9 },
  { label: 'Records correlated', value: '5', stage: 10 },
  { label: 'Exact matches', value: '3', stage: 12 },
  { label: 'Unresolved', value: '3', stage: 12 },
]

export const logLines: { text: string; color: string }[] = [
  { text: '$ innochecksum --version', color: '#8fa39f' },
  { text: 'innochecksum Ver 8.4.0', color: '#d3dedb' },
  { text: '$ innochecksum work/accounts.ibd', color: '#8fa39f' },
  { text: '  pages checked: 126   damaged: 0   exit 0', color: '#d3dedb' },
  { text: '$ ibd2sdi work/accounts.ibd', color: '#8fa39f' },
  { text: '  finance.accounts  @1 account_id  @2 customer_name  @3 balance', color: '#d3dedb' },
  { text: '$ ibd2sql work/accounts.ibd', color: '#8fa39f' },
  { text: '  3 rows extracted', color: '#d3dedb' },
  { text: '$ mysqlbinlog --base64-output=DECODE-ROWS -v work/binlog.000020', color: '#8fa39f' },
  { text: '  1 812 events decoded, 6 retained after table filter', color: '#d3dedb' },
  { text: '! R-COV-002: mysql-bin.index lists binlog.000019; not in evidence set', color: 'oklch(0.72 0.13 75)' },
  { text: '  grouping → TX-1452 committed (XID 8821)', color: '#d3dedb' },
...
export const recordHistory: Record<string, RecordHistory> = {
  'accounts:101': {
    ...
    steps: [
      { title: 'Earliest observed state', source: 'binlog.000018 pos 2210', note: 'First row image available in the imported log set.', color: GRY, fields: [{ col: 'balance', val: '5000.00', highlight: false }, { col: 'customer_name', val: 'A. Perera', highlight: false }] },
      { title: 'TX-1449 · UPDATE (rolled back)', source: 'binlog.000018 pos 2210', note: 'Transaction was rolled back; this change should not be durable.', color: GRY, fields: [{ col: 'balance', val: '5000.00 → 3500.00', highlight: true }] },
      ...
      { title: 'Physical state in .ibd', source: 'accounts.ibd via ibd2sql', note: 'Physical row holds 3500.00, matching the rolled-back value rather than the committed one.', color: RED, fields: [{ col: 'balance', val: '3500.00', highlight: false }, { col: 'result', val: 'Conflicting', highlight: false }] },
    ],
  },
  'accounts:205': {
    ...
    method: 'primary-key exact match',
    steps: [
      { title: 'Earliest observed state', source: 'binlog.000018 pos 2402', note: 'First row image available in the imported log set.', color: GRY, fields: [{ col: 'balance', val: '7500.00', highlight: false }] },
      ...
      { title: 'Physical state in .ibd', source: 'accounts.ibd via ibd2sql', note: 'Row not present in the extracted page set; no physical value to compare against.', color: ORG, fields: [{ col: 'balance', val: 'not observed', highlight: false }, { col: 'result', val: 'Unresolved', highlight: false }] },
    ],
  },
  'accounts:310': {
    ...
    method: 'primary-key exact match',
    steps: [
      { title: 'TX-1448 · UPDATE (incomplete)', source: 'binlog.000018 pos 1180', note: 'BEGIN observed with no terminator in the imported range.', color: GRY, fields: [{ col: 'customer_name', val: 'K. Fernando', highlight: false }] },
      { title: 'Coverage gap', source: 'binlog.000019 absent', note: 'Events between binlog.000018 pos 8814 and binlog.000020 pos 12 cannot be observed.', color: AMB, fields: [{ col: 'balance', val: 'not observed', highlight: false }] },
      { title: 'Physical state in .ibd', source: 'accounts.ibd via ibd2sql', note: 'Physical row exists but no log-derived value is available to compare.', color: ORG, fields: [{ col: 'balance', val: '2200.00', highlight: false }, { col: 'result', val: 'Unresolved', highlight: false }] },
    ],
  },
  'transfers:9001': {
    ...
    method: 'primary-key exact match',
    steps: [
      { title: 'Record absent', source: 'no prior event', note: 'No event referencing this key appears before TX-1452.', color: GRY, fields: [{ col: 'state', val: 'absent', highlight: false }] },
  ...
  'transfers:9002': {
    ...
    method: 'primary-key exact match',
    steps: [
      { title: 'Earliest observed state', source: 'binlog.000018 pos 2402', note: 'Row present with status PENDING.', color: GRY, fields: [{ col: 'status', val: 'PENDING', highlight: false }] },
      { title: 'TX-1449 · DELETE (rolled back)', source: 'binlog.000018 pos 2402', note: 'Delete recorded inside a transaction that was rolled back.', color: GRY, fields: [{ col: 'state', val: 'deleted → restored', highlight: true }] },
      { title: 'Physical state in .ibd', source: 'transfers.ibd via ibd2sql', note: 'Physical row holds COMPLETED, a value no observed event produced.', color: RED, fields: [{ col: 'status', val: 'COMPLETED', highlight: false }, { col: 'result', val: 'Conflicting', highlight: false }] },
    ],
  },
}

/**
 * What the timeline dot on each history step means.
 *
 * The dot carries evidential standing, not chronology: grey steps are observed
 * but not durable, teal is a committed change, and the closing step takes the
 * reconciliation outcome for the record. Every `HistoryStep.color` above must
 * appear here, or the step renders with an unexplained dot.
 */
export const historyLegend: { color: string; label: string; note: string }[] = [
  { color: GRY, label: 'Observed, not durable', note: 'Prior state, or a change from a rolled-back or incomplete transaction.' },
  { color: A, label: 'Committed change', note: 'Event in a transaction with an observed commit.' },
  { color: AMB, label: 'Coverage gap', note: 'Evidence for this span is absent; nothing can be observed.' },
  { color: GRN, label: 'Physical state agrees', note: 'The .ibd row matches the log-derived state.' },
  { color: RED, label: 'Physical state conflicts', note: 'The .ibd row holds a value no committed event produces.' },
  { color: ORG, label: 'Unresolved', note: 'Evidence is insufficient to compare the two states.' },
]
...
export const gaps: {
  ...
  { severity: 'Warning', tone: 'warn', title: 'Binary-log coverage gap', code: 'GAP-001', detail: 'mysql-bin.index lists binlog.000019, which is absent from the evidence set. Events between binlog.000018 pos 8814 and binlog.000020 pos 12 cannot be observed.', impact: 'Timeline may be incomplete', affects: 'finance.accounts, finance.transfers' },
...
export const rawLines: { text: string; color: string }[] = [
  { text: '### UPDATE `finance`.`accounts`', color: '#8fa39f' },
  { text: '### WHERE', color: '#d3dedb' },
  { text: '###   @1=101 /* INT meta=0 nullable=0 is_null=0 */', color: '#d3dedb' },
  { text: "###   @2='A. Perera' /* VARSTRING(100) */", color: '#d3dedb' },
  { text: '###   @3=5000.00 /* DECIMAL(12,2) */', color: '#d3dedb' },
  { text: '### SET', color: '#d3dedb' },
  { text: '###   @1=101', color: '#d3dedb' },
  { text: "###   @2='A. Perera'", color: '#d3dedb' },
