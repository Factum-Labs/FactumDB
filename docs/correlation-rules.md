...
| `R-ROLL-003` | All comparable fields agree | Every field that could be compared agreed, and at least one field could not be compared. This is the record-level reading of Strong: the key and all available comparable fields agree, but some values are unavailable. | Strong | notice |
| `R-ROLL-004` | Only part of the record could be compared | Some fields were compared and others could not be, with no conflict among those that were. | Partial | notice |
| `R-ROLL-005` | No field could be compared | Nothing about this record could be compared, so no conclusion is drawn about it either way. | Unresolved | warning |
